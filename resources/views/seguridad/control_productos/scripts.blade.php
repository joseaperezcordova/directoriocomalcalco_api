...
<script>
    $(document).ready(function () {
        $('#tabla').DataTable({
            ...
            order: [[0, 'desc']],
            ajax: "{{ route('seguridad.control-productos.data') }}",
            columns: [
                {data: 'id'},
                {data: 'fecha'},
                {data: 'producto'},
        ...
        $('#modalRecepcion').on('show.bs.modal', () => {
            ...
            $.get("{{ route('seguridad.control-productos.productos') }}", data => {
...
// 🔢 Actualizar total general
        ...
        $('#guardar').click(() => {
            ...
            $.post("{{ route('seguridad.control-productos.store') }}",
        ...
        $('#confirmarEntrega').click(function () {
            ...
            if ($btn.prop('disabled'))
                return; // si ya está deshabilitado, salir
            $btn.prop('disabled', true);
            $.post("{{ route('seguridad.control-productos.entrega') }}", {
                _token: '{{ csrf_token() }}',
                recepcion_id: id,
                cantidad: cantidad
            }, function () {
