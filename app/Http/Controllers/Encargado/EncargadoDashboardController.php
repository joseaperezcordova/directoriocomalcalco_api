...
class EncargadoDashboardController extends Controller {
    ...
    public function index() {
        return view('encargado.dashboard.index');
    }
    ...
    public function data() {
        ...
        $movimientos = DB::table('movimientos_detalle as m')
                ...
                ->where(function ($q) use ($puntoEncargado) {
                    $q->where('m.id_punto_venta_origen', $puntoEncargado)
                    ->orWhere('m.id_punto_venta_destino', $puntoEncargado);
                })
                ->orderByDesc('m.id')
        ...
        $recibidosHoy = DB::table('movimientos_detalle')
                ->where('id_punto_venta_destino', $puntoEncargado)
        ...
        $entregadosHoy = DB::table('movimientos_detalle')
                ->where('id_punto_venta_origen', $puntoEncargado)
                ->whereDate('created_at', now()->toDateString())
                ->sum('cantidad');
        ...
        $sub = DB::table('recepcion_detalles as r')
                ...
                ->where('r.id_punto_venta', $puntoEncargado)
