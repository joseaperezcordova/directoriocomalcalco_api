...
class EncargadoControlProductosController extends Controller {
    ...
    public function index() {
        ...
        $idSeguridad = User::where('rol', 'seguridad')->value('id_punto_venta');

        $puntosVenta = PuntoVenta::where('activo', 1)
                ->where('id', '!=', $idPropio)
                ->where('id', '!=', $idSeguridad)
                ->get();

        return view('encargado.control_productos.index', compact('puntosVenta'));
    }
    ...
    public function entrega(Request $request) {
        $request->validate([
            ...
            'id_punto_venta' => 'required|integer|exists:puntos_venta,id'
        ]);
        ...
        try {
            ...
            $puntoOrigen = auth()->user()->id_punto_venta;
            $puntoDestino = $request->id_punto_venta;
            ...
            DB::table('bitacora_producto')->insert([
                ...
                'id_punto_venta_origen' => $puntoOrigen,
                'id_punto_venta_destino' => $puntoDestino,
                'antes' => $antes_origen,
            ...
            $antes_destino = DB::table('inventario')
                            ->where('id_producto', $recepcion->id_producto)
                            ->where('id_punto_venta', $puntoDestino)
                            ->value('cantidad') ?? 0;
